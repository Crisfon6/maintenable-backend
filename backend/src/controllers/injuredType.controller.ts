...
export class InjuredTypeController implements IController {
  public path = "/injuredType";
  ...
  delete = async (req: Request, res: Response) => {
    const results = await InjuredType.findByIdAndUpdate(
      req.params.id,
      {
        state: false,
      },
  ...
  getById = async (req: Request, res: Response) => {
    const results = await InjuredType.findById(req.params.id);
    res.status(200).send({ items: results });
  };
  get = async (req: Request, res: Response) => {
    ...
    const q = { state: true };
    const total = await InjuredType.countDocuments(q);
    const users = await InjuredType.find({
      $or: [{ name: r }],
      $and: [{ state: true }],
    })
      .skip(Number(skip))
  ...
  create = async (req: Request, res: Response) => {
    const { name } = req.body;
    const newInjuredBodyPart = new InjuredType({
      name,
    });
